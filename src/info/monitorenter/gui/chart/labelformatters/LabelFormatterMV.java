...
import info.monitorenter.util.units.UnitGiga;
import info.monitorenter.util.units.UnitKilo;
import info.monitorenter.util.units.UnitMega;
 ...
 *
 */
public class LabelFormatterMV extends ALabelFormatter {
    ...
    static {
        ...
        for (final AUnit unit : UnitFactory.getInstance().getUnits(UnitSystemSI.getInstance())) {
            ...
            LabelFormatterMV.UNITS_2_POWER.put(unit, new Integer(power));
        }
    }
     ...
     */
    public LabelFormatterMV(final ALabelFormatter delegate) {
        super();
