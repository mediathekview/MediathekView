...
public class StarterClass_old {
    ...
    // Public
    //===================================
    public StarterClass_old(Daten daten) {
        this.daten = daten;
        starten = new Starten();
        starten.start();
    }
    ...
    private void notifyStartEvent(DatenDownload datenDownload) {
        Listener.notify(Listener.EREIGNIS_START_EVENT, StarterClass_old.class.getSimpleName());
        if (datenDownload != null) {
            if (datenDownload.quelle == DatenDownload.QUELLE_BUTTON) {
                Listener.notify(Listener.EREIGNIS_START_EVENT_BUTTON, StarterClass_old.class.getSimpleName());
            }
        }
    }
    ...
    private class Starten extends Thread {
        ...
        private void startStarten(DatenDownload datenDownload) {
            ...
            Listener.notify(Listener.EREIGNIS_ART_DOWNLOAD_PROZENT, StarterClass_old.class.getName());
