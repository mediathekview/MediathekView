...
public class GuiFilme extends PanelVorlage {
    ...
    public GuiFilme(DDaten d) {
        ...
        extra();
        setInfo(); //alter der Filmliste anzeigen
    ...
    public void filmSpeichern() {
        ...
        if (selRow >= 0) {
            ...
            ddaten.listeDownloads.addFilmDownload(film, Starts.QUELLE_DOWNLOAD);
    ...
    private void init() {
        ...
        DDaten.filmeLaden.addListener(new BeobTimer());
        //Tabelle einrichten
        ...
        ddaten.infoPanel.addMouseListener(new BeobMausProgramme());
...
//        jPanelExtra.addMouseListener(new BeobMausPgruppenDialog());
    ...
    private void setInfo() {
        ...
        if (jTable1.getModel() != null) {
            textLinks = jTable1.getModel().getRowCount() + " Filme";
        } else {
            textLinks = "0 Filme";
        }
        if (runs == 1) {
        ...
        // Infopanel setzen
        ddaten.infoPanel.setText(textLinks, textRechts);
    }
    // ############################################
