...
public class GuiDownloads extends PanelVorlage {
    ...
    private void init() {
        ...
        jTable1.setModel(new TModel(new Object[][]{}, DatenDownload.DOWNLOAD_COLUMN_NAMES));
    ...
    private void load() {
        ...
        getSpalten(jTable1);
        TModel tModel = new TModel(new Object[][]{}, DatenDownload.DOWNLOAD_COLUMN_NAMES);
            ...
            for (int i = rows.length - 1; i >= 0; --i) {
                ...
                if (dauerhaft) {
                    if (download.istAbo()) {
                        ...
                        ddaten.log.zeileSchreiben(download.arr[DatenDownload.DOWNLOAD_THEMA_NR],
        ...
        for (int i = 0; i < jTable1.getModel().getRowCount(); ++i) {
            ...
            if (s != null) {
                if (s.status >= Starts.STATUS_FERTIG) {
                    ddaten.listeDownloads.delDownloadByUrl(url);
                    ((TModel) jTable1.getModel()).removeRow(i);
                    --i;
                }
            }
        }
