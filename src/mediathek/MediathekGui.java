    ...
    public MediathekGui(String[] ar) {
        ...
        if (IoXmlLesen.einstellungenExistieren()) {
            ...
            if (ddaten.listePgruppe.size() == 0) {
                ...
                PanelImportStandardPgruppe panel = new PanelImportStandardPgruppe(ddaten);
                DialogLeer dialog = new DialogLeer(this, true, panel, "Videoplayer einrichten");
                panel.dialog = dialog;
    ...
    private void checkUpdate() {
        ...
        if (Boolean.parseBoolean(Daten.system[Konstanten.SYSTEM_UPDATE_SUCHEN_NR])) {
            if (!Daten.system[Konstanten.SYSTEM_UPDATE_DATUM_NR].equals(DatumZeit.getHeute_yyyyMMdd())) {
                DDaten.filmeLaden.checkProgrammVersion(ddaten, false /* nur bei neueer Version anzeigen */);
            }
        }
    }
